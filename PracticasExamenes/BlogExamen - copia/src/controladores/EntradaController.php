        ...
        */
        ...
        public function verEntrada() {
            ...
            if ($idEntrada) {
                ...
                $entadaVotada = $entradaModelo->getVotoEntrada($idEntrada, $_SESSION['user']->id);

                // Vista
                VistaController::mostrar('vistas/entradas/verUnaEntrada.php', ['entrada' => $entrada, 'usuario' => $usuario, 'categoria' => $categoria, 'cantidadVotos' => $cantidadVotos, 'entadaVotada' => $entadaVotada]);
            }
        }
        ...
        public function entradasMasVotadas() {
            $entradaModelo = new EntradaModelo();
            $entradas = $entradaModelo->getEntradasMasVotadas();

            // Vista
            VistaController::mostrar('vistas/entradas/entradasMasVotadas.php', ['entradas' => $entradas]);
        }

        public function votarEntrada() {
            if (Autenticacion::isUserLogged()) {
                $idEntrada = $_GET['id'];
                if ($idEntrada) {
                    $entradaModelo = new EntradaModelo();
                    $entradaModelo->votarEntrada($idEntrada);
                }
            } else {
                header('Location: ' . Parametros::$BASE_URL);
            }
            header('Location: ' . Parametros::$BASE_URL . 'Entrada/verEntrada&id=' . $idEntrada);
        }

        public function eliminarVotoEntrada() {
            if (Autenticacion::isUserLogged()) {
                $idEntrada = $_GET['id'];
                if ($idEntrada) {
                    $entradaModelo = new EntradaModelo();
                    $entradaModelo->eliminarVotoEntrada($idEntrada);
                }
            } else {
                header('Location: ' . Parametros::$BASE_URL . 'usuario/misVotos');
            }
            header('Location: ' . Parametros::$BASE_URL . 'usuario/misVotos');
        }
    }
